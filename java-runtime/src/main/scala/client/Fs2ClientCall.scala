...
class Fs2ClientCall[F[_], Request, Response] private[client] (
  ...
  def unaryToUnaryCall(message: Request, headers: Metadata)(implicit F: ConcurrentEffect[F]): F[Response] = {
    F.bracketCase(startListener(Fs2UnaryClientCallListener[F, Response](errorAdapter), headers))({ listener =>
  ...
  def unaryToStreamingCall(message: Request, headers: Metadata)(
    ...
    Stream
      .bracketCase(startListener(Fs2StreamClientCallListener[F, Response](call.request, errorAdapter), headers))(handleCallError)
